...
import { sendRequest, StatusChangeRequestPropType } from "../../util/WIP.util";
...
const StatusChangeRequestComponent: FC<StatusChangeRequestPropType> = ({
  ...
  const Submit = (value: { to: string; reviewed_by: string; }) => {
    setLoading(true);
    const data: { [key: string]: any } = {
      document_assignment_id: String(document_assignment.id),
      reviewed_by: `${getUserData().id}`,
      current_status: document_assignment.status,
      requested_status: value.to,
      revision: document_assignment.revision,
      type_on_status: "WIP",
      action_status: "Pending",
    };
    if (actionStatus) {
      data.id = documentStatusId;
    }
    sendRequest(data)
      .then(() => {
        handleOk();
          ...
          <div className="row">
            <div className="col-md-6">
              <Form.Item label="From">
                <Input value={document_assignment.status} disabled />
              </Form.Item>
            </div>
            <div className="col-md-6">
              <Form.Item
