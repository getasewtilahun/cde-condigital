...
import RequestForInformationService from "../../services/Communication/RequestForInformation.service";
import { toNumber } from "lodash";
import fs from "fs";
import logger from "../../utilities/loggers/winston";

class RequestForInformationController {
  /**
  ...
  static create(request: Request, response: Response) {
    ...
    if (result.isValid) {
      RequestForInformationService.create(data)
  ...
  static findAll(request: Request, response: Response) {
    ...
    RequestForInformationService.findAll(query)
      .then((result) => {
  ...
  /**
   * Find RequestForInformation By ID
   *
   * @param {Request} request
   * @param {Response} response
   */
  static findByID(request: Request, response: Response) {
    RequestForInformationService.findByID(request.params.id)
      .then((result) => {
        if (result) {
          response.json(result);
        } else {
  ...
  static update(request: Request, response: Response) {
    ...
    const result = Schema.validate(data);
    if (result.isValid) {
      RequestForInformationService.update(parseInt(data.id), data)
        .then(() => response.status(200).json(true))
        .catch((error) => {
          console.log(error);
  ...
  static delete(request: Request, response: Response) {
    ...
    RequestForInformationService.delete(id)
      .then(() => response.status(200).json(true))
      .catch((error) => response.status(error.statusCode).json(error.payload));
  }
}

export default RequestForInformationController;
